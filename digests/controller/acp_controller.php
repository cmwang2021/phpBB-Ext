	...
	public function display_options($mode, $u_action)
	{
		...
		switch ($mode)
		{
			...
			case 'digests_report':
				...
				// Validate the sort column against an allow-list and the direction against ASC/DESC so these
				// request values cannot be injected into the ORDER BY clause below.
				$sortable_fields = ['digests_report_id', 'date_hour_sent_utc', 'started', 'ended', 'mailed', 'skipped', 'execution_time_secs', 'memory_used_mb', 'cron_type'];
				$sort_field = $this->request->variable('sort_field', 'date_hour_sent_utc');
				if (!in_array($sort_field, $sortable_fields, true))
				{
					$sort_field = 'date_hour_sent_utc';
				}
				$sort_dir = (strtoupper($this->request->variable('sort', 'DESC')) === 'ASC') ? 'ASC' : 'DESC';
				...
				$sql_array = [
					...
					'ORDER_BY' => "{$sort_field} {$sort_dir}{$more_sorts}"
				];
				$sql = $this->db->sql_build_query('SELECT', $sql_array);
				$result = $this->db->sql_query_limit($sql, (int) $this->config['phpbbservices_digests_rows_per_page'], (int) $start);
				$rowset = $this->db->sql_fetchrowset($result);
