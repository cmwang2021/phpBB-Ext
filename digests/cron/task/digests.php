			...
			foreach ($posts_rowset as $post_row)
			{
				...
				else
				{
					...
					if (trim($this->config['phpbbservices_digests_strip_tags']) !== '')
					{
						...
						{
							$dom = new \DOMDocument();
							$dom->loadHTML('<?xml encoding="UTF-8">' . $post_text, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD); // declare UTF-8 so libxml parses multibyte characters correctly
							...
							$post_text = $dom->saveHTML($dom->documentElement); // saveHTML returns UTF-8; utf8_decode() removed (deprecated in PHP 8.2, removed in PHP 9, and it mangled non-Latin-1 text)
						}
					}
